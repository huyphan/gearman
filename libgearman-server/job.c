                       ...
                       gearman_return_t *ret_ptr)
{
  ...
  if (server_job == NULL)
  {
    ...
    server_job->forced_worker = forced_worker;
...
{
  gearman_server_worker_st *server_worker;
  gearman_server_job_st *server_job, *first_job;
  gearman_job_priority_t priority;
    ...
    if (server_worker->function->job_count != 0)
    {
      for (priority= GEARMAN_JOB_PRIORITY_HIGH;
                 priority != GEARMAN_JOB_PRIORITY_MAX; priority++)
        {
          if (server_worker->function->job_list[priority] != NULL)
          {
            first_job = server_worker->function->job_list[priority];
            server_job = first_job;
            do {
              if ( (server_job->forced_worker == 0) || (server_job->forced_worker == server_con->con.fd) )
              {
                found = true;
                break;
              }
            ...
            while ((server_job!=first_job) && (server_job!=NULL));
          }
          if (found) break;
        }
        if (found) break;
  ...
  {
    first_job = server_job->function->job_list[priority];
    while (first_job->function_next != server_job) first_job = first_job->function_next;
    first_job->function_next = server_job->function_next;
  }
...
gearman_return_t gearman_server_job_queue(gearman_server_job_st *job)
{
  ...
  /*
  if ( (job->function->job_list[job->priority] != NULL) && (job->function->job_end[job->priority] == NULL) )
    printf("DETECTED NULL\n");
  else printf("QUEUE OK\n");
*/
